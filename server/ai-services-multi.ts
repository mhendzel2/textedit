import { getAIResponse, getOptimalProvider, type AIProvider } from "./ai-providers";
...
export async function performDevelopmentalEdit(
  ...
  const optimalProvider = provider === 'openai' ? getOptimalProvider('structure') : provider;
  
  const prompt = `You are a professional developmental editor. Your task is to improve the following text according to the given instructions. Focus on:
...
Please provide your edited version and explain the changes you made. Format your response as a valid JSON object with the following structure:
{
  ...
  "changes": [
    {
      ...
      "explanation": "brief explanation of why this change was made"
    }
  ],
  "summary": "overall summary of the developmental changes made"
}`;

  return getAIResponse(prompt, optimalProvider, 0.3);
}
...
export async function performLineEdit(
  ...
  const optimalProvider = provider === 'openai' ? getOptimalProvider('creative-writing') : provider;
...
Please provide your edited version and track all changes made. Format your response as a valid JSON object with the following structure:
{
  ...
  return getAIResponse(prompt, optimalProvider, 0.2);
}
...
Provide 5 different editing prompts as a JSON object with a single key "suggestions" containing an array of strings. Example: {"suggestions": ["prompt1", "prompt2"]}`;
  ...
  const result = await getAIResponse(prompt, getOptimalProvider('creative-writing'), 0.7);
...
export async function analyzeMetaphors(
  content: string, 
  provider: AIProvider = 'google'
): Promise<MetaphorAnalysis> {
...
Provide your analysis as a JSON object with this structure:
{
  ...
  return getAIResponse(prompt, getOptimalProvider('metaphors'), 0.3);
}
...
Provide your analysis as a JSON object with this structure:
{
  ...
  return getAIResponse(prompt, getOptimalProvider('character-voice'), 0.4);
}
...
Provide your analysis as a JSON object with this structure:
{
  ...
  return getAIResponse(prompt, getOptimalProvider('world-building'), 0.3);
}
...
Provide your analysis as a JSON object with this structure:
{
  ...
  return getAIResponse(prompt, getOptimalProvider('pacing'), 0.4);
}
...
Provide your analysis as a JSON object with this structure:
{
  ...
  return getAIResponse(prompt, getOptimalProvider('structure'), 0.4);
}
...
Provide your analysis as a JSON object with this structure:
{
  ...
  return getAIResponse(prompt, getOptimalProvider('themes'), 0.4);
}